...
describe Jobs::VotingEnsureConsistency do
  it "ensures consistency" do
    ...
    expect(DiscourseVoting::Vote.where(user: user).map(&:topic_id)).to eq([one_vote_topic.id, two_vote_topic.id])
    expect(DiscourseVoting::Vote.where(user: user2).map(&:topic_id)).to eq([two_vote_topic.id])
